...
public class ScalingUnhingedScavengerBingoCrystalObjective extends CrystalObjective implements INamedObjective {
    ...
    public ScalingUnhingedScavengerBingoCrystalObjective(float objectiveProbability, int sealCount) {
        this.objectiveProbability = objectiveProbability;
        this.sealCount = sealCount;
    }

    public ScalingUnhingedScavengerBingoCrystalObjective() {
        this.sealCount = 1;
    }

    @Override
    public Optional<Integer> getColor(float v) {
        return Optional.of(9159498);
    }
    ...
    public String getCrystalName() {
        return getHeight() + "x" + getWidth() + " Unhinged Collector Crystal";
    }
    ...
    public static float posX(int width){
        var scale = ModOptions.OBJECTIVES_HUD_SETTINGS.getValue().getSettings("bingo").getScale();
        return -1.5f * width/scale;
    }

    public static float posY(int height){
        var scale = ModOptions.OBJECTIVES_HUD_SETTINGS.getValue().getSettings("bingo").getScale();
        return (2 - height *1.3f)/ scale;
    }
}
