...
import xyz.iwolfking.woldsvaults.datagen.lib.GodTreeBuilder;

import java.io.IOException;
import java.nio.file.Path;

/**
 * Emits the god constellation tree definitions consumed by {@code GodTrees} at runtime. The Idona
 * layout is the a30 draft exported from {@code redesign/greed-rework/tree-drafts/idona_tree_v4.py}
 * in the pack repo, and the Wendarr hourglass, Velara wreath and Tenos hoard-and-chest are the
 * drafts from {@code wendarr_tree.py}, {@code velara_tree.py} and {@code tenos_tree.py} next to
 * it, exported by the matching {@code export_<god>_wiring.py}; coordinates are screen pixels with
 * +y down, and edges are the undirected lattice connections. Stat helper methods keep each node's
 * display name, effect id and icon in exactly one place, and a stat the sheet lists as a pair is
 * two helpers, the shallow band and the {@code _ii} deep one. Ultra Rampaging and the two shelved
 * extraction nodes are declared through the disabled builder methods until their mechanics land.
 */
public class ModGodTreesProvider implements DataProvider {
    ...
    public ModGodTreesProvider(DataGenerator generator) {
        this.generator = generator;
    }

    @Override
    public void run(HashCache cache) throws IOException {
        save(cache, "idona", idona());
        save(cache, "wendarr", wendarr());
        save(cache, "velara", velara());
        save(cache, "tenos", tenos());
    }

    private void save(HashCache cache, String god, GodTreeBuilder tree) throws IOException {
        Path path = this.generator.getOutputFolder().resolve("data/woldsvaults/god_trees/" + god + ".json");
        DataProvider.save(GSON, cache, tree.build(), path);
    }
