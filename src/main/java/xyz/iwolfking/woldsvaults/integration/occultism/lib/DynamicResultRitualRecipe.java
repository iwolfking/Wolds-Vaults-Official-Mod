...
public class DynamicResultRitualRecipe extends RitualRecipe {
    ...
    public boolean matches(Level level, BlockPos goldenBowlPosition, ItemStack activationItem) {
        ...
        if (this.getEntityToSacrifice() != null) {
            double range = 8.0;
            AABB area = new AABB(goldenBowlPosition).inflate(range);


            List<LivingEntity> nearbyEntities = level.getEntitiesOfClass(
                    LivingEntity.class,
                    area,
                    entity -> entity.isAlive() && entity.getType().is(this.getEntityToSacrifice())
            );
    ...
    public static class Serializer extends RitualRecipe.Serializer {
        ...
        public DynamicResultRitualRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
            ...
            if ("entity_to_sacrifice".equals(this.jsonKey) && json.get("entity_to_sacrifice").isJsonPrimitive()) {
                JsonObject sacrificeObj = new JsonObject();
                sacrificeObj.addProperty("tag", customId.toString());
                ResourceLocation entityId = OccultismEntityTagHelper.convertSacrificeTagToEntityId(customId);
                sacrificeObj.addProperty("display_name", "entity." + entityId.getNamespace() + "." + entityId.getPath());
