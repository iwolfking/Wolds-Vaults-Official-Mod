...
public class MobAdditionalDamageOnHitModifier extends VaultModifier<MobAdditionalDamageOnHitModifier.Properties> {
    private final DamageSource source;
    public MobAdditionalDamageOnHitModifier(ResourceLocation id, MobAdditionalDamageOnHitModifier.Properties properties, Display display, DamageSource source) {
                            ...
                            if (!context.hasTarget() || context.getTarget().equals(player.getUUID())) {
                                if (!(world.random.nextFloat() >= this.properties.onHitApplyChance)) {
                                    if(this.properties.isPercentageOfAttack()) {
                                        player.hurt(this.source, event.getAmount() * this.properties.getAmount());
                                    }
                                    else {
                                        player.hurt(this.source, this.properties.getAmount());
                                    }
                                }
                            }
                        }
